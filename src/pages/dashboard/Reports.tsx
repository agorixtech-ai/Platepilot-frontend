              ...
              <tbody>
                {isLoading ? (
                  ...
                  <tr>
                    ...
                    >
                      No data for this date range. Try adjusting the dates.
                    </td>
                  </tr>
                ) : (
                  daily.map((row) => (
                    ...
                    >
                      <td className="px-4 py-3 font-semibold text-foreground text-[11px]">
                        {fmtDate(row.date)}
                      </td>
                      ...
                      <td className="px-4 py-3 tabular-nums text-success font-medium">
                        {aed(row.gross_profit)}
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={cn(
                            "tabular-nums font-medium",
                            row.food_cost_pct > 40
                              ? "text-destructive"
                              : row.food_cost_pct > 25
                                ? "text-warning"
                                : "text-success",
                          )}
                        >
                ...
                <tfoot>
                  <tr className="border-t-2 border-border/60 bg-muted/20">
                    ...
                    <td className="px-4 py-3 font-bold tabular-nums text-foreground">
                      {aed(daily.reduce((s, r) => s + r.sales, 0))}
                    </td>
                    <td className="px-4 py-3 font-bold tabular-nums text-foreground">
                      {daily.reduce((s, r) => s + r.orders, 0)}
                    </td>
                    <td className="px-4 py-3 font-bold tabular-nums text-foreground">
                      {aed(daily.reduce((s, r) => s + r.purchases, 0))}
                    </td>
                    <td className="px-4 py-3 font-bold tabular-nums text-warning">
                      {aed(daily.reduce((s, r) => s + r.net_cost, 0))}
                    </td>
                    <td className="px-4 py-3 font-bold tabular-nums text-success">
                      {aed(daily.reduce((s, r) => s + r.gross_profit, 0))}
                    </td>
                    <td className="px-4 py-3 font-bold tabular-nums text-foreground" colSpan={2}>
                      {/* not meaningful to sum pcts */}
                    </td>
                  </tr>
                </tfoot>
              )}
        ...
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
          <Card className="border border-border/60 bg-card shadow-sm lg:col-span-2">
            <CardHeader className="px-5 pb-2 pt-5">
              ...
              <p className="text-[11px] text-muted-foreground mt-0.5">
                Highest revenue items in this period
              </p>
            </CardHeader>
